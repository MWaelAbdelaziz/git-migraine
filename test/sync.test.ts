...
describe('sync', () => {
  ...
  it('undoes a migration that no longer exists on the new branch', async () => {
    ...
    expect(fs.existsSync(path.join(repo, 'migrations/20240102-b.cjs'))).toBe(false);
  });
  ...
  it('does nothing on a file checkout (branchFlag 0)', async () => {
    ...
    const code = await sync({ oldRef, newRef, branchFlag: '0', config: testConfig() });
    expect(code).toBe(0);
  ...
  it('skips on the initial checkout (zero oldRef)', async () => {
    const newRef = await git(['rev-parse', 'HEAD']);
    const code = await sync({ oldRef: ZERO_REF, newRef, branchFlag: '1', config: testConfig() });
    expect(code).toBe(0);
    expect(callsLog()).toEqual([]);
  });
  ...
  it('skips while a merge is in progress', async () => {
    ...
    const code = await sync({ oldRef, newRef, branchFlag: '1', config: testConfig() });
    expect(code).toBe(0);
    expect(callsLog()).toEqual([]);
  });
